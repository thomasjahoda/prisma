...
- **Benchmarking**: Comprehensive performance benchmarks using Benchmark.js with CodSpeed integration. See `docs/benchmarking.md` for full documentation. Key commands:
  ...
  - `packages/type-benchmark-tests/test.ts` only exits non-zero for generation/runtime exceptions; benchmark threshold failures are printed as `❌ Failed` in the summary but currently do not flip the process exit code.
