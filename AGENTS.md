...
- **Build & tooling**: Typescript-first repo with WASM/Rust assets (downloaded by `@prisma/engines`). Multiple `tsconfig.*` drive bundle vs runtime builds. Lint via `pnpm lint`, format via `pnpm format`. Maintenance scripts live in `scripts/` (e.g. `bump-engines.ts`, `bench.ts`, `ci/publish.ts` orchestrates build/test/publish flows). Build configuration uses esbuild via `helpers/compile/build.ts` with configs in `helpers/compile/configs.ts`. Most packages use `adapterConfig` which bundles to both CJS and ESM with type declarations.
  - `packages/migrate` currently emits fresh declaration files to `packages/migrate/dist/src/**` via `tsc --project tsconfig.build.json`, but the build does not clean `dist`, so stale files can linger under `packages/migrate/dist/migrate/src/**`. `packages/migrate/package.json` still points `types` at `dist/migrate/src/index.d.ts`, so verify timestamps before trusting that tree when debugging type mismatches.
  - `pnpm pack` for these workspace packages emits internal workspace dependency versions as `0.0.0` in the packed `package.json`. Any fork-publish workflow that stages tarballs needs to rewrite those dependency values to a real published Prisma base version (for example `7.6.0-dev.6`) or consumers will try to install nonexistent `0.0.0` packages.
  - For fork publishes that are meant to be consumed via package-manager aliases, only rename the published package `name`. Keep internal dependency keys and runtime imports pointing at the official `prisma` / `@prisma/*` names so aliasing in the consumer app can remap them consistently.
- **Benchmarking**: Comprehensive performance benchmarks using Benchmark.js with CodSpeed integration. See `docs/benchmarking.md` for full documentation. Key commands:
  ...
  - When `packages/type-benchmark-tests/test.ts` is given a positional filter, it now prefilters benchmark files before running `prisma generate`, so irrelevant schema directories are skipped entirely instead of regenerating all fixtures.
  ...
  - For `prisma-client-ts` simplified-mode type-check benchmarks, most of the remaining whole-file instantiation cost currently comes from importing `generated/client.ts` itself. A temporary `import './generated/client'` benchmark in `basic-ts-simplified` measured essentially the same instantiation count as `const client: PrismaClient<'info'>` plus `client.user.findUnique(...)`, so further large wins likely require a lighter main client entrypoint rather than more delegate/result tweaks.
  - For multi-file benchmark fixtures, point `prisma.config.ts` at a dedicated `schema/` subdirectory instead of the fixture root; `prisma-client-js` custom output writes a `generated/schema.prisma`, and if the schema root includes `generated/`, subsequent runs can recursively load the generated schema and produce duplicate model errors.
  - `packages/type-benchmark-tests/test.ts` exits non-zero when an attest benchmark or a `*.type-check-benchmark.ts` file fails.
...
- **Testing patterns**:
  ...
  - `packages/client/src/__tests__/types/types.test.ts` now targets `test.client.ts` for engine-specific type fixtures; older `getClientEngineType()`-based helpers are stale in current Prisma.
  - The simplified type-test mode currently relies on the process-global env hack `PRISMA_HACK_GENERATOR_CONFIG_DISABLETYPINGSUPPORTFORHEAVYFEATURES`; run those tests sequentially, not concurrently, or fixtures can interfere with each other.
  - `packages/client/src/__tests__/integration/happy/not-so-exhaustive-schema*` fixtures are Prisma 7 style: keep datasource URLs out of `schema.prisma`, use `prisma.config.ts` for connection config, and import `getDMMF` from `@prisma/client-generator-js`.
  - `packages/client` integration helpers such as `src/utils/generateInFolder.ts` import `@prisma/client-generator-js` through the workspace package entrypoint, which resolves to `packages/client-generator-js/dist`, not `src`. After changing generator source files, rebuild `@prisma/client-generator-js` before trusting generated-client snapshots or fixtures.
